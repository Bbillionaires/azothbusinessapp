...
export default function EventDetailScreen() {
  ...
  const event = {
    id,
    title: 'Downtown Vendor Market',
    type: 'vendor_market',
    description: 'Join us for our monthly vendor market featuring over 50 local vendors, artisans, and food trucks. Live music, community giveaways, and special appearances from local business owners.',
    start_at: new Date(Date.now() + 86400000 * 3).toISOString(),
    end_at: new Date(Date.now() + 86400000 * 3 + 14400000).toISOString(),
    address: '121 W Forsyth St',
    city: 'Jacksonville',
    state: 'FL',
    image_url: null,
    is_free: true,
    ...
    points_reward: 25,
    business_name: 'Downtown Jacksonville Partnership',
  };

  const attendancePct = (event.current_attendees / event.max_attendees) * 100;

  const handleRSVP = (status: 'going' | 'interested') => {
    setRsvpStatus(prev => prev === status ? 'none' : status);
  };
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.coverContainer}>
          ...
          <View style={styles.typeBadge}>
            <Text style={styles.typeBadgeText}>
              {EVENT_TYPE_ICONS[event.type]} {event.type.replace('_', ' ').replace(/\b\w/g, c => c.toUpperCase())}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.body}>
          ...
          <Text style={styles.organizer}>by {event.business_name}</Text>
          ...
          <View style={styles.detailsCard}>
            ...
            <View style={[styles.detailRow, styles.detailBorder]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.detailValue}>{event.city}, {event.state}</Text>
                <TouchableOpacity
            ...
            <View style={[styles.detailRow, styles.detailBorder]}>
              ...
              <View>
                ...
                <Text style={styles.detailValue}>{event.is_free ? '🎁 Free Entry' : `$${event.price}`}</Text>
              </View>
            </View>
